...
cat("🧬 Loading Prairie Genomics Suite R Shiny\n")
cat("=========================================\n")

required_packages <- c(
  "shiny", "shinydashboard", "shinyWidgets", "DT", 
  "plotly", "ggplot2", "dplyr", "readr"
)

optional_packages <- c(
  "DESeq2", "readxl", "RColorBrewer", "pheatmap", "EnhancedVolcano"
)

# Load required packages
for (pkg in required_packages) {
  if (!require(pkg, character.only = TRUE, quietly = TRUE)) {
    stop(paste("Required package", pkg, "is not installed. Please run install.R first."))
  }
}
...
for (pkg in optional_packages) {
  if (require(pkg, character.only = TRUE, quietly = TRUE)) {
    if (pkg == "DESeq2") bioc_available <- TRUE
    if (pkg == "readxl") excel_support <- TRUE
    if (pkg %in% c("RColorBrewer", "pheatmap", "EnhancedVolcano")) enhanced_plots <- TRUE
  } else {
    cat(paste("Optional package", pkg, "not available - some features may be limited\n"))
  }
}
