...
visualization <- function(input, output, session, values) {
  ...
  output$volcano_plot <- renderPlotly({
    ...
    if (!is.null(input$highlight_genes) && input$highlight_genes != "") {
      ...
      highlight_data <- results_df[results_df$gene %in% highlight_list, ]
  ...
  output$heatmap_plot <- renderPlot({
    req(values$deseq2_results, values$filtered_data)
    
    # Get top significant genes
    sig_genes <- values$deseq2_results[values$deseq2_results$significant == TRUE, ]
    ...
    expr_data <- values$filtered_data[sig_genes$gene, ]
  ...
  output$pca_plot <- renderPlotly({
    req(values$filtered_data, values$annotation_data)
    
    # Perform PCA
    pca_data <- t(values$filtered_data)
